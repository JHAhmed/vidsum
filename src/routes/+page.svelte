	...
	async function handleSubmit() {
		...
		try {
			const response = await fetch('/api/summarize', {
				...
				body: JSON.stringify({ url: youtubeUrl, segments: numSegments, length: summaryLength, type: summaryType })
			});
			if (!response.ok) {
	...
	async function saveSummary() {
		...
		try {
			const response = await fetch('/api/save', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ title: title, content: summary })
			});
			if (!response.ok) {
				const errorData = await response.json();
				throw new Error(errorData.message || `HTTP error! status: ${response.status}`);
...
<div
	class="mx-auto w-full max-w-6xl space-y-8 rounded-xl bg-gray-100 p-6 shadow-xl backdrop-blur-md sm:p-10 dark:bg-slate-900 dark:shadow-slate-800/20"
>
	...
	<form on:submit|preventDefault={handleSubmit} class="space-y-6">
		...
		<div class="my-6 flex w-full flex-col sm:flex-row sm:space-x-2 md:space-x-4">
			<div class="w-full">
				<Tabs
