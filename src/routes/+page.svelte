...
<div class="flex flex-col items-center justify-center">
	<div
		animate-in
		use:animateIn={{ blur: 2, y: 2, delay: 0 }}
		class="mx-12 w-full max-w-6xl space-y-8 rounded-xl mt-16 bg-gray-100 p-4 py-8 shadow-xl backdrop-blur-md sm:p-6 md:p-10 dark:bg-slate-900 dark:shadow-slate-800/20"
	>
		<header animate-in use:animateIn={{ blur: 2, y: 2, delay: 0.2 }} class="text-center">
		...
		<section
			animate-in
			use:animateIn={{ blur: 2, y: 2, delay: 0.4 }}
		...
		>
			<div class="max-w-3xl space-y-4 text-center">
				<p class="text-xl text-slate-700 dark:text-slate-200">
					VidSum lets you quickly summarize YouTube videos into easy-to-read notes using AI. Just
					paste a video link and get concise, actionable summaries in seconds.
				</p>
				<p class="text-base text-slate-600 dark:text-slate-400">
					Save your summaries, organize your notes, and boost your learning efficiency. Try it now —
					no signup required!
				</p>
			</div>
			<div class="mt-4 flex flex-col justify-center gap-4 sm:flex-row">
				<a
					href="/app"
					class="inline-flex items-center rounded-lg bg-sky-500 px-6 py-3 font-semibold text-white shadow transition hover:bg-sky-600"
				>
					Go to App
				...
				{#if !data.user}
					<a
						href="/auth/"
						class="inline-flex items-center rounded-lg bg-gray-200 px-6 py-3 font-semibold text-sky-600 shadow transition hover:bg-gray-300 dark:bg-slate-800 dark:text-blue-300 dark:hover:bg-slate-700"
					>
						Login
...
<footer
	animate-in
	use:animateIn={{ blur: 2, y: 2, delay: 0.6 }}
	class="my-8 text-center text-sm text-slate-500 dark:text-slate-400"
>
